...
## 2026-09-04 — M2.4b Task 1–2：图库详情抽屉与可读生成摘要

- 状态：图库布局、按需详情抽屉、ComfyUI prompt 摘要与受控大图预览已接入；真实桌面视觉验收尚未完成。
- 交互：默认显示多列媒体图库，详情不再常驻占用第三栏；单击资产后打开详情，关闭按钮和 Escape 可返回图库，旧的异步详情响应不会覆盖后选中的资产。
- 摘要：新增 `summarizeComfyPrompt`，仅解析详情 IPC 已提供的 prompt JSON；按 KSampler 的真实链接提取正/负提示词、Checkpoint、采样器、调度器、步数、CFG、种子、去噪、宽高。无法追溯的字段标记“未解析”，不从文件名、目录或 JSON 顺序猜测。
- 展示：详情先显示紧凑的生成摘要，原始 Prompt 与 Workflow 收进默认折叠的“高级数据”；保留 `png_metadata` 来源标识和无数据/无效状态。
- 预览：新增 UUID-only `get_asset_preview` 命令与 `cache/previews/v1` 受控 WebP 缓存，最长边限制 2048px；详情中的“放大预览”打开可关闭、Escape 关闭并恢复焦点的弹窗。Asset Protocol 仅开放缩略图和预览缓存范围。
- 测试先行：布局/抽屉已有红灯回归；摘要测试先因模块不存在失败，再覆盖完整链路、断链和非法 JSON 后转绿。
- 验证：前端 26 个测试文件、107 项通过，生产构建通过；Rust 全量测试通过（2 个需真实环境变量的只读 smoke 保持忽略），`cargo fmt --check`、Clippy `-D warnings` 与 `git diff --check` 通过。自动化已覆盖命令标识与源路径不泄露契约；真实桌面视觉矩阵、缓存命中/失效的真实媒体 smoke 尚未完成。
- 下一步：在真实 WebView2 中验收多列图库、抽屉和预览弹窗，再补缓存命中/mtime 失效的本机媒体 smoke。
...
- 根因：列表 SQL 固定按 `normalized_path` 升序，因此 `input` 目录会因路径字母序排到 `output` 前，不代表素材或生成结果的先后。
- 查询契约：`query_assets` 新增可选 `sort`，旧调用省略时也默认 `modified_desc`；前端始终明确提交排序值。默认“最近修改”把空修改时间排在有效时间之后，并用索引时间、路径和 ID 解决相同时间的稳定分页顺序。
- 交互：资产工具栏新增“排序方式”，提供最近/最早修改、路径 A–Z / Z–A、文件最大/最小；切换后回到第 1 页。排序发生在 SQLite 查询层，跨页结果一致，不会只在当前前端页重排。
- 安全边界：本项仅读取 ComfyNeko SQLite 索引，不会扫描、复制、移动、重命名、删除或改写 input、output、模型和工作流源文件。
- 测试先行：新增 Rust 测试先确认“较新的 output 仍被较旧 input 排在后面”的目标行为失败；新增前端测试先确认请求未携带排序字段且切换排序不会回到第 1 页，然后实施最小查询契约与 UI 状态改动。
- 验证：前端 22 个测试文件、95 项通过，生产构建通过；Rust 全量测试 70 项通过、2 项需真实环境变量的只读 smoke 按条件忽略，`cargo fmt --check` 与 Clippy `-D warnings` 通过。`tauri build --debug --no-bundle` 因运行中的 `target\\debug\\comfyneko.exe` 被 Windows 拒绝覆盖（os error 5）而未完成。
- 下一步：在当前打开的桌面窗口选择“公司ComfyUI”，确认首屏最新 output、较新的 input 与现有图片按“最近修改”混排正确；再检查窄窗口下两个下拉框的换行与英文短文案。

## 2026-09-04 — M2.3c 图片缩略图与受控缓存实现

- 状态：代码、自动化测试、生产构建、Tauri debug 构建与浏览器视觉矩阵完成并已逐任务提交推送；真实公司环境的首次缓存生成等待用户在最新桌面窗口中手动确认。
- 范围：首版只生成图片缩略图，支持 PNG、JPEG、WebP、BMP 和 TIFF；视频和音频继续使用类型占位，不引入 FFmpeg。
- 安全边界：前端只提交资产 ID；Rust 重新验证资产、环境、媒体类型、在场状态和允许根目录。缓存只写入 `app_local_data_dir()/cache/thumbnails/v1/`，Tauri Asset Protocol 只允许访问该缓存范围，不开放 ComfyUI 输入或输出目录。
- 生命周期：缓存键包含资产 ID、当前文件大小、修改时间和算法版本；源文件变化后自动使用新版本，成功生成后只清理同资产的旧缓存。源文件始终只读且永不作为缓存清理目标。
- 交互：图片卡片使用 `IntersectionObserver` 懒加载，固定预览尺寸避免布局跳动；不支持、缺失、损坏和生成失败统一回退现有占位，不阻断资产列表。
- 后端：新增资产按 ID 读取、路径守卫、缩略图领域状态、图片解码上限、640px WebP 生成、缓存命中、源文件变化失效、并发临时文件隔离、稳定命令错误码和受限 Asset Protocol。支持格式精确限制为 PNG、JPEG/JPG、WebP、BMP、TIFF/TIF。
- 前端：新增 Tauri/browser 双环境 API、旧请求隔离、图片解码错误回退和可见范围懒加载；资产卡保持既有 4:3 比例，视频和音频不发起缩略图请求，不新增渐变、徽章或布局高度。
- 测试先行：资产读取、安全路径、支持边界、未知资产、生成、复用、损坏/超大图片、源变化、并发、命令映射、API 转换、浏览器回退、观察器、旧响应和页面集成均先确认目标行为缺失时失败，再完成最小实现。
- 验证：前端 22 个测试文件、94 项测试通过，生产构建成功；Rust workspace 73 项通过、2 项真实环境 smoke 保持忽略，格式检查和 Clippy `-D warnings` 通过；Tauri debug `--no-bundle` 构建成功。Playwright 六尺寸矩阵无页面、资产页或列表横向溢出，图片加载和 4:3 比例稳定，视频/音频占位保留，控制台 0 error；截图位于 `output/playwright/image-thumbnail-cache/`。
- 本机数据：应用数据库已有“公司ComfyUI”环境，当前在场记录为图片 68、音频 1、模型 19、工作流 8。新版本首次显示图片前缓存目录不存在且 WebP 数量为 0，符合按可见卡片懒生成设计。
- 提交：`badb84a`、`dfc04e4`、`fad76cb`、`9a28e53`、`d09a817`、`3e2b9f6` 已推送到 `fix/visionhub-environment-ui`。
- 下一步：用户在最新桌面窗口选择“公司ComfyUI”，确认首屏与滚动懒生成、缓存目录、占位回退和源文件不变；通过后进入媒体详情、PNG metadata 与生成记录关联。

## 2026-09-04 — 资产库职责与 `media_only` 查询边界

- 状态：媒体资产库职责、前端筛选与后端查询边界已实现并完成自动化验收；真实 Tauri debug 程序已重新构建，等待用户继续手动观感验收。
- 页面职责：“资产管理”只展示图片、视频和音频；模型文件由“模型管理”负责，工作流 JSON 由“工作流库”负责，`custom_nodes` 与节点插件由“节点管理”负责。模型、工作流和节点插件不得为了复用底层索引而混入媒体资产筛选或卡片。
- 索引与展示：M2.2 扫描和 SQLite 仍可保存模型、工作流等广义资产记录，为独立功能页提供共享数据基础；这不改变资产库的媒体专属边界。
- 查询契约：`query_assets` 的 `media_only=true` 只允许 `image`、`video`、`audio` 返回，并继续与环境、类型、根目录、目录、在场状态、搜索和分页条件组合。省略或关闭 `media_only` 保留通用内部查询能力，不能被资产库页面用于扩大展示范围。
- 前端：快捷筛选只保留“全部、图片、视频、音频”，目录分类只保留“全部资产、输入素材、生成结果”；搜索文案不再提模型，所有资产库查询都固定提交 `media_only=true`。
- 后端：请求层新增可选 `media_only`，领域查询层使用明确布尔值；SQLite 总数和分页列表使用同一媒体范围条件，从查询结果和总数中同时排除模型与工作流，而不是只隐藏前端按钮。
- 数据核对：本机应用数据库当前有图片 68、音频 1、模型 19、工作流 8；媒体范围结果为 69，模型与工作流不计入资产库数量。
- 验证：前端 20 个测试文件、81 项测试通过，生产构建通过；Rust workspace 测试全部通过（2 项需真实环境变量的只读 smoke 保持忽略），`cargo fmt --check`、Clippy `-D warnings` 与 Tauri debug `--no-bundle` 构建通过。Playwright 检查中文亮色 1504/1180/560/320/240px 和英文暗色 1180/320/240px，根文档与资产页无横向溢出，媒体筛选和目录分类均无模型/工作流入口，查询记录全部为 `media_only=true`，控制台 0 error。截图位于 `output/playwright/asset-media-boundary/`。
- 下一步：接入受控缩略图缓存与真实媒体预览，继续优化英文工具栏宽度和超窄窗口分类栏细节；模型管理、工作流库和节点管理分别建立自己的查询与页面增量。
