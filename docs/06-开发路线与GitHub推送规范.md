...
- ~~M1.1：建立隔离开发工作树、Rust 工作区与可序列化的环境领域类型。~~
- ~~M1.2：实现只读路径发现、根目录识别与机器可读诊断。~~
- ~~M1.3：实现有超时上限的 Python 与仅本机 API 探测。~~
- ~~M1.4：实现环境档案持久化、共享 UI 基础与环境绑定向导。~~

### M2：本地资产数据层

建立 SQLite 数据库、数据库迁移、路径白名单、后台任务、缓存和增量扫描基础。先实现索引正确性与取消恢复，再接 UI。

- ~~M2.1：建立资产领域模型、根目录角色分类、只读递归发现和 SQLite 增量索引。~~
- M2.2：实现后台扫描任务、取消、持久化检查点、恢复和完整批次失效记录处理。
- M2.3：实现分页查询、筛选接口、缓存目录与资产库 UI 接入基础。
...
- 每次推送同时更新 `docs/DEVELOPMENT_LOG.md`，记录目标、改动、验证、风险、下一步与提交标识；完成的 M1 子项必须在本文件保留原文并用删除线标记。
