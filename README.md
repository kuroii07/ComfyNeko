...
- 环境设置：通用设置、加速与架构、模型路径、环境变量已按独立页签组织。环境控制台现有固定状态页首、键盘可访问页签与减少动效降级；通用/模型路径已升级为“说明列 + 统一配置面板”，并有八类模型目录的内存草案映射。已建立纯前端安全草案域，可解析 `KEY=VALUE` 环境变量、识别重复和格式错误；它不调用 Tauri、不触碰 ComfyUI 或系统环境变量。模型路径继续支持原生选择、打开和只读自动发现；加速参数与启动环境变量未来只会以可预览、备份和撤销的 ChangePlan 形式开放。
