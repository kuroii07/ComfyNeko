...
- 应用导航：已建立固定宽度的四字单层功能栏，包含首页总览、模型管理、资产管理、工作流库、提示词库和节点管理；环境管理与偏好设置固定在侧栏底部。“资产管理”已接入真实扫描控制页，其余未开发页面继续明确标记为规划中。
...
- 资产查询：~~M2.3a 后端分页与基础筛选接口~~ 已完成。查询只读取 ComfyNeko SQLite，按环境隔离，支持资产类型、根目录类别、目录及在场/失效状态组合筛选；使用稳定路径排序、1 起始页码、默认每页 50 条且最多 100 条，并返回列表所需的名称、目录、大小、时间和状态字段。
- 当前目标：继续 M2.3b，建立受控缓存目录并接入最小资产列表；模型的 Checkpoint、Diffusion Model、LoRA、VAE、Text Encoder 等子分类留到后续独立细分。在明确批准写操作前，继续保持已绑定 ComfyUI 目录只读。
